...
#if 0
#ifndef CONFIG_SLIC_LE88221
    ucTmpVersion = kerSysGetGPIO(BP_GPIO_8_AH);
    ucTmpVersion = ((ucTmpVersion << 1) | kerSysGetGPIO(BP_GPIO_7_AH));    
    ucTmpVersion = ((ucTmpVersion << 1) | kerSysGetGPIO(BP_GPIO_6_AH)); 
#else
    ucTmpVersion = kerSysGetGPIO(BP_GPIO_7_AH);
#endif
#endif
    ucTmpVersion = kerSysGetGPIO(BP_GPIO_7_AH);
    return ucTmpVersion;        
...
/**************************************************************************
 * 函数名  :   GetHarewareType
 * 功能    :   获取硬件类型,由于华为自研的硬件有预留GPIO6-GPIO8来区分不同的
 *             硬件版本(如: HG52VAG,HG52VAGB,HG51VAG,HG51VAGI),这样对于华为
 *             自研硬件不需要手动设置Board ID.
 *
 * 输入参数:   无           
 * 输出参数:   无 
 *             
 * 返回值  :    NOT_SUPPROT_GET_VERSION :   非华为自研硬件, 按FLASH中设置的Board ID处理
 *              0,1,4 :   华为自研硬件, 将硬件参数强制定向到"HW6358GW_B"
 *
 * 作者    :    yuyouqing42304
 * 修改历史:    2006-07-10创建      
 ***************************************************************************/
...
{  
...
#if 0	
    #if defined(SUPPORT_HARDWARE_VERSION)
    {        
        ...
        /* 若是HG520 DEMO，则按之前在CFE里设置的BOARD ID处理 */        
        if (((0 == strcmp(nvramData.szBoardId, "HW6358GW_A")) || (0 == strcmp(nvramData.szBoardId, "96358M")))
            && (HG52VAGB != ucBoardVersion) && (HG51VAG != ucBoardVersion) && (HG51VAGI != ucBoardVersion))
        {            
            return NOT_SUPPROT_GET_VERSION;
        }
        
        /* 若是HG52VAG,HG52VAGB,HG51VAG或HG51VAGI，则将BOARDID指定为"HW6358GW_B" */
        else if ((HG52VAG == ucBoardVersion) || (HG52VAGB == ucBoardVersion) || (HG51VAG == ucBoardVersion)
                 || (HG51VAGI == ucBoardVersion) )
        {            
            return ucBoardVersion;
        }
        else 
        {            
            return NOT_SUPPROT_GET_VERSION;
    ...
    #elif defined(CONFIG_SLIC_3210) || defined(CONFIG_SLIC_3215) ||defined(CONFIG_SLIC_LE88221)
...
void kerSysFlashInit( void )
{
    ...
    /*
     * start of init added by y42304 20060708: 解决华为自研硬件不需要手动设置
     * BOARD ID，而是通过读GPIO6-GPIO8动态确认硬件版本来初始化硬件 
     */
     ucHardwareType = GetHarewareType();
    
    /*s48571 modify for VDF HG553 hardware porting*/
    /*note: there is no define for the HG553 hardware, so use the CONFIG_SLIC_LE88221 instead!*/
//#if defined(CONFIG_SLIC_3210) || defined(CONFIG_SLIC_3215)|| defined(CONFIG_SLIC_LE88221)
#if 1
     switch(ucHardwareType)
     {
        ...
        case HG55MAGC:
        {
            printk("\n Board id is set HW550\n");
            BpSetBoardId("HW550");    
            break;
        }
        case HG55MAGD:
        {            
            printk("\n Board id is set HW550_FXO\n");
            BpSetBoardId("HW550_FXO");    
            break;
        }        
        default: 
        {
    ...
    BpSetBoardId( nvramData.szBoardId );       
     ...
     switch(ucHardwareType)
     {
         ...
         case HG51VAGI:            
            ...
            BpSetBoardId("HW6358GW_B");             
            break;
         default: 
            BpSetBoardId( nvramData.szBoardId );            
            break;            
     }    
     /* end of init added by y42304 20060708 */
#endif
    ...
    /*end l65130 20080328 增加2个分区 Fix Avail, 将原有 VA 及 SP 分别使用独立的 sector */
...
//        }
		...
		printk("Image flasing on block: %d\n", blk_start);
