...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if ( proto == PPP_IP) {
		data = skb->data;
		timestamp = on_demand_filter(data);
	}
...
ppp_create_interface(int unit, int *retp)
{
	...
	if ((unit+1) > OFFSET1)
	{
	    num1 = unit / (OFFSET2*OFFSET1);
	    num2 = (unit - OFFSET2*OFFSET1*num1) / OFFSET1;
	    num3 = (unit - OFFSET2*OFFSET1*num1) - OFFSET1*num2;
	    sprintf(dev->name, "ppp_%d_%d_%d", num1, num2, num3);
	}
	else
#endif
	sprintf(dev->name, "ppp%d", unit);
//	printk("ppp creat interface : %s\r\n",dev->name);
