...
#define IN_BWMODEM     16      //定义带宽调节因子，经计算，当usb bulk传输缓冲为64字节，调节因子定义为64时，usb1.1时带宽约为5M
#define OUT_BWMODEM     9       //定义带宽调节因子，经计算，当usb bulk传输缓冲为64字节，调节因子定义为64时，usb1.1时带宽约为3M
#define BWOTHER         2       //diag数据包约为70－80字节，增加输入buf来加快传输速度
...
extern unsigned int g_ulNumOfDataCard;
...
static void proc_user_umts_create(void)
{
    ...
    proc_umts->write_proc = (read_proc_t *)proc_user_umts_wr;
    proc_umts->read_proc = (write_proc_t *)proc_user_umts_rd;
...
static int serial_open (struct tty_struct *tty, struct file * filp)
{
	...
	//if (port->open_count == 1) {//removed by z67625
...
#if 1 //def  CONFIG_ATP_SUPPORT_RT3052
    //printk("\n==file:%s,line:%d==\n",__FILE__,__LINE__);
...
#if 0
#undef HW_CARD_PID
#define HW_CARD_PID(pid,portString)    \
{ .pid= (pid),\
			       ...
			       const struct usb_device_id *id)
{
    ...
    /*Start of modified by lvxin 00135113 for support hspa 2009年4月30日*/
        ...
        if((0x12d1 == id->idVendor)||(0xd112 == id->idVendor))
        {
            /*用于适配华为数据卡*/
            printk("\nThis is hw device\n");
            g_ulNumOfDataCard ++;
...
/* HUAWEI HGW s48571 2008年3月2日" SIP通话过程中拔插HSPA导致死机 add begin:*/
    struct tty_struct *tty = NULL;
/* HUAWEI HGW s48571 2008年3月2日" SIP通话过程中拔插HSPA导致死机 add end.*/
	dbg ("%s", __FUNCTION__);
...
;//            tty->driver_data = NULL;
  ...
  /*start of modified by lvxin 00135113@20090821 to close ttyUSBX*/
    while(atomic_read(&serial->kref.refcount) > 0)
    {
        if (serial) 
        {
            for (i = 0; i < serial->num_ports; ++i) 
  ...
  /*End of modified by lvxin 00135113@20090821 to close ttyUSBX*/
	--iUsbTtyState;
    if (iUsbTtyState==0)
    {
