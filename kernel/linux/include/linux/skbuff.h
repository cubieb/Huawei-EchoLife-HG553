...
#endif
#define QOS_8021P_0_MARK        8
#define QOS_8021P_TRANSPARENT   9
/*port for 553*/
#if 0
#define MARK_ATM_PRI_MASK               0x00000007
#define QOS_FILTER_MARK                     0x000000F0
#define QOS_DEFAULT_MARK                             0x100
#define MARK_VLAN_1P_MASK                0x00000F00     /*ebt_mark.c,this mark is no  use*/
//#define MARK_DSL_8021P_MASK             0x0000F000            no use
#define MARK_VOICE_FILTER                 0x00001000            /*临时规避语
                                                                  音报文如imq
                                                                  导致阻塞的
                                                                  问题*/
#define QOS_XTM_HIGH_PRI                    0x00010000
#define MARK_8021P_FILTER_MASK      0x00F00000
#define MARK_ROUTE_MASK                   0x01000000
#define MARK_LAN_MASK                         0xF0000000
/*wmm 左移7位*/
#else
#define MARK_ATM_PRI_MASK               0x00000007
#define QOS_FILTER_MARK                     0x000000F0
#define MARK_VLAN_1P_MASK                0x00000F00     /*ebt_mark.c,this mark is no  use*/
//#define MARK_DSL_8021P_MASK             0x0000F000            no use
#define MARK_8021P_FILTER_MASK      0x0000F000
#define QOS_XTM_HIGH_PRI                    0x00010000
#define QOS_DEFAULT_MARK                  0x00020000
#define MARK_ROUTE_MASK                   0x00040000
#define MARK_VOICE_FILTER                 0x00080000            /*临时规避语
                                                                  音报文如imq
                                                                  导致阻塞的
                                                                  问题*/
#define MARK_LAN_MASK                         0x00F00000
/*wmm 左移7位*/
#endif
...
#if defined(CONFIG_RTL8190) || defined(CONFIG_USB_RTL8192SU_SOFTAP)
...
/*此宏已关闭，但为保留原有skbuff结构不变（为了不影响加速），所以仍保留这些参
  数.*/
//#ifdef CONFIG_ATP_LAN_VLAN
/*提供给 struct sk_buff,
与 struct skb_vlan_tag的区别是:
1) struct skb_vlan_tag 给QOS Vlan优先级使用.
2) 本数据成员 给LAN侧VLAN使用。
使用方法:当接受到一个报文以后，判断是否带有VLAN ID。
a) 如果报文中带有VLAN ID，则去掉报文的VLAN ID，并将信息赋予给本结构中的TCI.
b) 如果报文中没有带VLAN ID，则使用端口 struct port_vlanInfo中的pvid值。*/

    unsigned int         vlan_id_flags;           /* 它有两个属性值: ATP_VLAN_ID_TAG 和 ATP_VLAN_ID_PVID*/ 
    unsigned short       h_vlan_id;               /* Encapsulates priority and VLAN ID */
    unsigned short       h_vlan_nook;            /* 钩子点 */
//#endif
	char			*extif;
...
#ifdef CONFIG_NETFILTER
    /*add by heary for prsite*/
    unsigned long		nfmark; /*not use now,we use "mark" instead*/
...
#if  defined(CONFIG_BRIDGE_NETFILTER) || defined(CONFIG_MIPS_BRCM)
