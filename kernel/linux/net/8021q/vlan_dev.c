...
#define TC_HIGH_PRIORITY  0x6000
		  ...
		  struct packet_type* ptype, struct net_device *orig_dev)
{
	...
	__be16 proto;
#ifdef CONFIG_ATP_SUPPORT_HG612
    	//~~ begin: vlan spec for bt
	int vlan_of_lan;
	int prio_of_lan;
	unsigned short tci_of_lan;
	//~~ e n d: vlan spec for bt
#endif
...
#ifdef CONFIG_ATP_SUPPORT_HG612
        //~~ begin: vlan spec for bt
        /* check if vlan of lan is set, if so, replace the vlan of skb ,
        if not, remove the orig vlan . Same method with 1p proto.*/
        vlan_of_lan = vlan_dev_get_vlan_of_lan(skb->dev, skb);
        prio_of_lan = vlan_dev_get_prio_of_lan(skb->dev, skb);

        if (vlan_of_lan >= 0 && vlan_of_lan <= VLAN_VID_MASK)
        {
            tci_of_lan = vlan_of_lan & VLAN_VID_MASK;
            // HANDLE 1P proto
            if (prio_of_lan >= 0 && prio_of_lan <= 7)
            {
                prio_of_lan = prio_of_lan << 13 & 0xe000;
                tci_of_lan |= prio_of_lan;
            }

            // replace VLAN & PRIO
            vhdr->h_vlan_TCI = tci_of_lan;
        }
        //~~ e n d: vlan spec for bt
        else
        {
	...
	if (ntohs(proto) >= 1536) {
		 ...
		 */
            ...
            if (vlan_of_lan >= 0 && vlan_of_lan <= VLAN_VID_MASK)
            {
                ...
                // the vlan tag has been replaced...
            }
            //~~ e n d: vlan spec for bt
            else
            {
...
#ifdef CONFIG_ATP_SUPPORT_HG612
//~~ begin: vlan spec for bt
int vlan_dev_get_vlan_of_lan(struct net_device* dev,
							  struct sk_buff* skb)
{
    return (VLAN_DEV_INFO(dev)->vlan_of_lan);
}

int vlan_dev_get_prio_of_lan(struct net_device* dev,
							  struct sk_buff* skb)
{
    return (VLAN_DEV_INFO(dev)->prio_of_lan);
}

int vlan_dev_set_vlan_of_lan(char *dev_name, int vlanoflan)
{
	struct net_device *dev = dev_get_by_name(dev_name);
	if (dev) {
        if (dev->priv_flags & IFF_802_1Q_VLAN) {
            if (vlanoflan >= 0 && vlanoflan <= VLAN_VID_MASK) {
                VLAN_DEV_INFO(dev)->vlan_of_lan = vlanoflan;
                dev_put(dev);
                printk("+vlan_dev_set_vlan_of_lan: set vlan ok! (%d)\n", vlanoflan);
                return 0;
            }
            else {
    		    printk("invalid vlanoflan\n");
            }
        }
        else {
            printk(KERN_ERR 
...
int vlan_dev_set_prio_of_lan(char *dev_name, int priooflan)
{
	struct net_device *dev = dev_get_by_name(dev_name);
	if (dev) {
        if (dev->priv_flags & IFF_802_1Q_VLAN) {
            if (priooflan >= 0 && priooflan <= 7) {
                VLAN_DEV_INFO(dev)->prio_of_lan = priooflan;
                dev_put(dev);
                printk("+vlan_dev_set_prio_of_lan: set vlan ok! (%d)\n", priooflan);
                return 0;
            }
            else {
    		    printk("invalid priooflan\n");
            }
        }
        else {
            printk(KERN_ERR 
...
extern int g_vlan_adapt[8];
#define IS_NAS0(dev)  (('n' == dev->name[0]) && ('0' == dev->name[3]))
#endif
...
#ifdef CONFIG_ATP_QOS_NETFILTER
    if ((skb->mark & MARK_8021P_FILTER_MASK) && (IS_NAS0(VLAN_DEV_INFO(dev)->real_dev)))
    {
        index = VLAN_DEV_INFO(dev)->vlan_id;		
        //if ((g_vlan_adapt[index] < 4095) && (g_vlan_adapt[index] >= 0) && (g_vlan_adapt[index] != 0xff))
        if ((index < 4095) && (index >= 0))
        {
            ucPrio = (skb->mark & MARK_8021P_FILTER_MASK) >> 12;
            if ((ucPrio < QOS_8021P_0_MARK) && (ucPrio >= 0))//0-7重标记
    		{
    			uc802Prio = ucPrio;
                tag802q = (((uc802Prio << 13) & 0xE000) | (index & VLAN_VID_MASK));
                //skb = __vlan_put_tag( skb , tag802q );            
    		}
            else if (QOS_8021P_0_MARK == ucPrio) //零标记
            {
				uc802Prio = 0;
                tag802q = (((uc802Prio << 13) & 0xE000) | (index & VLAN_VID_MASK));
                //skb = __vlan_put_tag( skb , tag802q );            
            }
    		else if (QOS_8021P_TRANSPARENT == ucPrio) /*透传*/
    		{
    		    /* void */
				tag802q = 0xFF;
    		}
   ...
   /* original code */
	...
	if (veth->h_vlan_proto != __constant_htons(ETH_P_8021Q))
	{
      ...
      if (skb->vtag & 0x1000) {
	   ...
	   veth_TCI = VLAN_DEV_INFO(dev)->vlan_id;
#endif
        if ((skb->mark & QOS_XTM_HIGH_PRI) && (IS_NAS0(VLAN_DEV_INFO(dev)->real_dev)))
        {
            veth_TCI |= TC_HIGH_PRIORITY;
        }
        else
        {
		    veth_TCI |= vlan_dev_get_egress_qos_mask(dev, skb);
...
#ifdef CONFIG_ATP_QOS_NETFILTER
		if((skb->mark & MARK_8021P_FILTER_MASK) 
				&& (IS_NAS0(VLAN_DEV_INFO(dev)->real_dev))
				&& tag802q != 0xFF)
		{
			veth_TCI = tag802q;//replace veth_TCI if 802.1p remark matched

		}
        else if (skb->mark & 0x0100)
        {
            veth_TCI = ((veth_TCI & 0x1fff) | ((skb->mark & 0x0e00) << 4));
        }
#endif                
#ifdef CONFIG_ATP_SUPPORT_HG612
            //~~ begin: vlan spec for bt
            /* check if vlan of lan is set, if so, replace the vlan of skb if the vlan does exists,
            if not, add the vlan of wan. Same method with 1p proto.*/
            vlan_of_lan = vlan_dev_get_vlan_of_lan(dev, skb);
            prio_of_lan = vlan_dev_get_prio_of_lan(dev, skb);
            if ((0 == __vlan_get_tag(skb, &orig_TCI))&& (vlan_of_lan >= 0)
                 && (vlan_of_lan <= VLAN_VID_MASK ))
            {
                // SHOULD check whether the original vlan id is equal to vlan of lan ? 
                // Leave it behind, replace it always.
                struct vlan_ethhdr *veth = (struct vlan_ethhdr *)skb->data;
                veth->h_vlan_TCI = veth_TCI;
            }
            //~~ e n d: vlan spec for bt
            else
            {
