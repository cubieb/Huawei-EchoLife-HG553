...
static void atp_delete_neigh(struct neigh_table *tbl)
{
	int i;
	struct neighbour *n, **np;
...
next_elt:
    ...
    /*清除路由缓存*/
    ip_rt_run_flush_by_arp(0);
    
	return;
}

void atp_find_neigh(struct neigh_table *tbl)
{
	unsigned long now = jiffies;
	int i;
	int delflags = 0;
	...
	unsigned long ulTimeMin = tbl->parms.gc_staletime;

	struct neighbour *atpneigh = NULL;
	...
	while ((n = *np) != NULL) 
    {
        ...
        if ((atomic_read(&n->refcnt) == 1)
            ...
            && (ulTimeMin > (n->parms->gc_staletime - (now - n->used))))
        {
            ulTimeMin = n->parms->gc_staletime - (now - n->used);
            atpneigh = n;
        }
...
next_elt:
    ...
    if (NULL != atpneigh)
    {
        write_lock(&atpneigh->lock);

        atpneigh->nud_state = NUD_FAILED;
        atpneigh->dead = 1;
        delflags = 1;
        
        write_unlock(&atpneigh->lock);
