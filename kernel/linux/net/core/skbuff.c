 ...
 *
 *	Version:	$Id: skbuff.c,v 1.2 2010/07/10 12:36:08 l43571 Exp $
 *
 *	Fixes:
			    ...
			    int fclone, int node)
{
	...
	if(size != (NET_SKB_PAD+0x700))
		size+=8;	//cathy, for wlan tkip
#endif	
...
/*add by heary for prsite*/
   ...
   C(vtag_save);
	...
	if(skb->extif) {
	  ...
	  if(extif) {
	    memcpy(extif, skb->extif, BCM_SNOOPING_BUFSZ);
	    n->extif = extif;
	  }
	}
/* start of add by d00107688 to support keep vlan from layer 2 */
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#if 0
	n->nat_cache_hit = 0;
...
/*add by heary for prsite*/
	...
	extif = kmalloc(BCM_SNOOPING_BUFSZ, GFP_ATOMIC);
	  if(extif) {
	    memcpy(extif, old->extif, BCM_SNOOPING_BUFSZ);
	    new->extif = extif;
	  }
	}
...
//#ifdef CONFIG_ATP_LAN_VLAN
    new->vlan_id_flags = old->vlan_id_flags;
    new->h_vlan_id = old->h_vlan_id;
    new->h_vlan_nook = old->h_vlan_nook;
//#endif
#ifdef CONFIG_USB_RTL8192SU_SOFTAP
	new->fcpu = 1;
#endif
