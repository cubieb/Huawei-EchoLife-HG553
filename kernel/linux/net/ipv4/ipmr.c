 ...
 *
 *	Version: $Id: ipmr.c,v 1.1 2010/07/06 06:45:41 c65985 Exp $
 *
 *	Fixes:
...
static int ipmr_mfc_delete(struct mfcctl *mfc)
{
	...
	for (cp=&mfc_cache_array[line]; (c=*cp) != NULL; cp = &c->next) {
		if (c->mfc_origin == mfc->mfcc_origin.s_addr &&
		    ...
		    /*解决组织模块配置多个wan接口(真实只有一个接口可以组播)组播问题, by h00101845 20080225*/
...
//	cache = ipmr_cache_find(skb->nh.iph->saddr, skb->nh.iph->daddr);
    ...
    line = MFC_HASH(skb->nh.iph->daddr, skb->nh.iph->saddr);

    mr_ifindex = ipmr_find_vif(dev);
	for (cache = mfc_cache_array[line]; cache; cache = cache->next) {
		if (cache->mfc_origin == skb->nh.iph->saddr && cache->mfc_mcastgrp == skb->nh.iph->daddr
            && cache->mfc_parent == mr_ifindex)
		{
			break;
	    }
	}
    if (NULL == cache)
    {
        /* 注意，这里skb->nh.iph有没有可能为空指针呢???? */
        line = MFC_HASH(skb->nh.iph->daddr, htonl(0x00000000));
        for (cache = mfc_cache_array[line]; cache; cache = cache->next)
        {
            if (skb->nh.iph->daddr == cache->mfc_mcastgrp && cache->mfc_parent == mr_ifindex)
            {
                break;
            }
        }
...
int ipmr_get_route(struct sk_buff *skb, struct rtmsg *rtm, int nowait)
{
    ...
    int mr_ifindex = 0;
      ...
      /* start of AU4D01318 by d00107688 2009-02-04 网关存在一条static的wan与一LAN口绑定，开启igmp
         * proxy，并存在一条PPPOE手动拨号的wan，网关串口出现CPU0，重启后仍然出现 
         */
    int flag = 0;
          /* end of AU4D01318 by d00107688 2009-02-04 网关存在一条static的wan与一LAN口绑定，开启igmp
         * proxy，并存在一条PPPOE手动拨号的wan，网关串口出现CPU0，重启后仍然出现 
         */
...
//	cache = ipmr_cache_find(rt->rt_src, rt->rt_dst);
    ...
    line = MFC_HASH(rt->rt_dst, rt->rt_src);

    mr_ifindex = ipmr_find_vif(dev);

    
	for (cache = mfc_cache_array[line]; cache; cache = cache->next) {
		if (cache->mfc_origin == rt->rt_src && cache->mfc_mcastgrp == rt->rt_dst
            && cache->mfc_parent == mr_ifindex)
		{
			break;
	    }
    ...
    if (NULL == cache)
    {
      /* start of AU4D01318 by d00107688 2009-02-04 网关存在一条static的wan与一LAN口绑定，开启igmp
        ...
        if (skb->nh.iph)
        {
            ...
            for (cache = mfc_cache_array[line]; cache; cache = cache->next)
            {
                if (rt->rt_dst == cache->mfc_mcastgrp && cache->mfc_parent == mr_ifindex)
                {
                    break;
                }
            }
