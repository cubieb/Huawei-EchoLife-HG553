...
static void trigger_ports(struct nf_conn *ct, int dir, int idx)
{
	...
	for (iport = inport[idx*3+1]; iport <= inport[idx*3+2]; iport++) {
		if ((exp = nf_conntrack_expect_alloc(ct)) == NULL) {
			DEBUGP("nf_nat_pt: nf_conntrack_expect_alloc error\n");
			return;
		}
		if (inport[idx*3] == PT_PROTO_TCP)
			...
			iproto = ct->tuplehash[dir].tuple.dst.protonum;
		memset(&exp->tuple.src.u3, 0, sizeof(exp->tuple.src.u3));
		exp->tuple.src.u.all = 0;
		exp->tuple.src.l3num = AF_INET;
		memset(&exp->tuple.dst.u3, 0, sizeof(exp->tuple.dst.u3));
		exp->tuple.dst.u3.ip = ct->tuplehash[!dir].tuple.dst.u3.ip;
		exp->tuple.dst.u.all = htons(iport);
		exp->tuple.dst.protonum = iproto;
		memset(&exp->mask.src.u3, 0, sizeof(exp->mask.src.u3));
		exp->mask.src.u.all = 0;
		exp->mask.src.l3num = 0xFFFF;
		memset(&exp->mask.dst.u3, 0, sizeof(exp->mask.dst.u3));
		...
		exp->saved_proto.all = htons(iport);
		exp->dir = !dir;
		if(nf_conntrack_expect_related(exp) == 0) {
			DEBUGP("nf_nat_pt: expect incoming connection to "
			       "%u.%u.%u.%u:%hu %s\n",
			       NIPQUAD(exp->tuple.dst.u3.ip), iport,
		...
		struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	if ((nfct_help(ct))->expecting) { /* Already triggered */
		return NF_ACCEPT;
	}
	...
	for (i = 0; i < entries; i++) {
		...
		if (iface[i] && strcmp(iface[i], (*pskb)->dst->dev->name))
			continue;
...
static void fini(void)
{
	...
	for (i = 0, h = pt; i < valid_ports; i++, h++) {
		if (h->list.next == NULL) /* Not registered */
			break;
		DEBUGP("nf_nat_pt: unregister helper for port %hu\n",
...
static int count_inport(void)
{
   ...
   for( i=0; i<entries; i++) 
   {
      ...
      {
         printk("nf_nat_pt: inport range is greater than maximum number %d"
                "Remaining ports are not processed.\n", PT_MAX_EXPECTED);
         inport[i*3+2] = inport[i*3+1] + PT_MAX_EXPECTED - 1;
      }
   }
   ...
   count_inport();
	...
	for (i = 0; i < entries; i++) {
		for (port = outport[i*3+1]; port <= outport[i*3+2]; port++ ) {
			...
			h->mask.src.u.all = 0xFFFF;
			h->mask.src.l3num = 0xFFFF;
			h->help = help;

			DEBUGP("nf_nat_pt: register helper for port %hu for "
