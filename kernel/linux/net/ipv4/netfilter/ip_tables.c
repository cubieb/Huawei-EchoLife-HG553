...
#define FWINV(bool,invflg) ((bool) ^ !!(ipinfo->invflags & invflg))
	...
	if (FWINV(ret != 0, IPT_INV_VIA_IN)) {
        ...
        /* Look for lan ifname matches; this should unroll nicely. */
    	for (i = 0, ret = 0; i < IFNAMSIZ/sizeof(unsigned long); i++) {
    		ret |= (((const unsigned long *)lanindev)[i]
    			^ ((const unsigned long *)ipinfo->iniface)[i])
    			& ((const unsigned long *)ipinfo->iniface_mask)[i];
    	}
	     ...
	     struct xt_table *table)
{
    ...
    lanindev = (*pskb)->lanindev ? (*pskb)->lanindev->name : nulldevname;
	...
	do {
		...
		IP_NF_ASSERT(back);
        /* add lan interface (eth0.5) in iptables 20090217 */
		if (ip_packet_match(ip, indev, outdev, lanindev, &e->ip, offset)) {
