...
/*START ADD: ftp attack should use log info 2009-6-27 */
static void
ipt_log_packet2(unsigned int hooknum,
	       const struct sk_buff *skb,
	       const struct net_device *in,
	       const struct net_device *out,
	       ...
	       const char *prefix)
{
	spin_lock_bh(&log_lock);
	printk(level_string);
	printk("104602 丢弃可能的非法连接 ");
	dump_packet(loginfo, skb, 0);
	printk("\n");
	spin_unlock_bh(&log_lock);
}
/*END ADD: ftp attack should use log info 2009-6-27 */
...
/*START ADD: ftp attack should use log info 2009-6-27 */    
	...
	ipt_log_packet2(hooknum, *pskb, in, out, loginfo, level_string, NULL);
