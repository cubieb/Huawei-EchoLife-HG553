...
static inline int match_ipland(const struct ipt_misc_info *info, struct sk_buff* skb)
{
    ...
    if (iph->saddr == iph->daddr) 
    {
        ...
        if ((0x7f000001 != iph->saddr) &&
          /*modify by chenzhigang 2008年7月25日,
          由于Spain版本默认出厂ip是192.168.0.1,
          且ipland默认打开,
          语音在进行装备测试时,使用的上行ip为此192.168.0.1*/
         (0xC0A80001 != iph->saddr))
        {
            return 1;
        }
...
static inline int match_dns_url(const struct ipt_misc_info *info, struct sk_buff* skb)
{
    ...
    unsigned int ret = 0;
    int i;
/* BEGIN: Added by weishi kf33269, 2010/10/22   PN:URL过滤功能，不需要区分大小写，因为所有的浏览器都是可以自动识别字母的大小写的*/
    int lencmp = 0;
   int cmpcount = 0;
   int infourllen = strlen(info->dns_url);
  /* END:   Added by weishi kf33269, 2010/10/22 */
   ...
   /* BEGIN: Added by weishi kf33269, 2010/10/22   PN:URL过滤功能，不需要区分大小写，因为所有的浏览器都是可以自动识别字母的大小写的*/
        lencmp = strlen(url) - infourllen;
	for(cmpcount = 1;cmpcount <= lencmp;cmpcount++)
	{
		if(!strnicmp(url+cmpcount,info->dns_url,infourllen))
		{
			  kfree(urls);
			 ret = 1;
			 return ret;
...
static inline int match_rtp2(const struct ipt_misc_info *info, struct sk_buff* skb)
{
    const struct iphdr *iph = (void *)skb->nh.iph;
    struct udphdr udp_hdr;
    struct rtphdr rtp_hdr;
        ...
        int offset, int *hotdrop)*/
{
    ...
    {
        //DEBUGPC("NOW, come here, begin to match the data......");
