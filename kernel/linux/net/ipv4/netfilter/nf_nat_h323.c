...
/****************************************************************************/
/* 将给定的字段的地址设置成addr和port指出的新的地址和端口 */
			  ...
			  struct nf_conntrack_expect *exp)
{
    ...
    DEBUGP("now, nat for: src[%u.%u.%u.%u:%d] ----> dst[%u.%u.%u.%u: %d]",
...
/* RTP/RTCP的预期nat信息， 这个函数还是存在问题，需要进行修改 */
#if 0
...
#endif
{
	...
	if (i >= H323_RTP_CHANNEL_MAX) {
        DEBUGP("nf_nat_h323: out of expectations\n");
		    ...
		    struct nf_conntrack_expect *exp)
{
    ...
    /* 因为 H.225.0 消息可能需要重传，因此这里做了优化 */
	/* Check existing expects */
	if (info->sig_port[dir] == port)
		nated_port = ntohs(info->sig_port[!dir]);

    /* 注意，如果该端口已经被占用了，那么此时需要修改端口从而使得终端以为服务器就是指定为该端口 */
	/* Try to get same port: if not, try to change it. */
	for (; nated_port != 0; nated_port++) {
		exp->tuple.dst.u.tcp.port = htons(nated_port);
		if (nf_conntrack_expect_related(exp) == 0)
	...
	/* Modify signal 修改数据包中的端口245 */
	if (set_h225_addr(pskb, data, dataoff, taddr,
			  &ct->tuplehash[!dir].tuple.dst.u3,
			  htons(nated_port)) == 0) {
		/* Save ports  保存真正的端口 */
		info->sig_port[dir] = port;
		info->sig_port[!dir] = htons(nated_port);   /* 这是修改之后的端口，将对应端口保留下来从而使得转发的时候可以对报文做NAT */
	} else {
		nf_conntrack_unexpect_related(exp);
		return -1;
	}
			       ...
			       struct nf_conntrack_expect *this)
{
	...
	if (this->tuple.src.u3.ip != 0) {	/* Only accept calls from GK */
        ...
        DEBUGPC("now, nat for: src[%u.%u.%u.%u:%d] ----> dst[%u.%u.%u.%u: %d]",
...
if (!is_local_addr(new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3))
{
    ...
    DEBUGP("now, nat for: src[%u.%u.%u.%u:%d] ----> dst[%u.%u.%u.%u: %d] to src[%u.%u.%u.%u:%d]",
        NIPQUAD(new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip), 
        (new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.udp.port),
        NIPQUAD(new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip), 
        (new->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port),
