...
#if 0
#define PRSITE_STATIC_DEBUGP(format, args...) do{if(lPrsiteDebug)printk(format, ## args);}while(0)
#else
...
{
    int ret = 0;
    PRSITE_STATIC_DEBUGP("prsite init is start...\n");
...
int prsite_add_static(struct sk_buff *skb)
{
    ...
    unsigned int addr = skb->nh.iph->daddr;
    ...
    {
        psite = kmalloc(sizeof(struct power_site), GFP_KERNEL);
        if(NULL == psite)
        {
               ret = 1;
               goto out;
    ...
    paddr = kmalloc(sizeof(struct affined_addr), GFP_KERNEL);
    if(NULL == paddr)
    {
...
void http_dump_power_sites(void)
{
    ...
    printk("\n\t wifi_url <%s>", (g_stPrsiteUrlInfo.ac_wifi_url));
}
...
int http_set_appdata(struct sk_buff **pskb, void *pdata, int len)
{
    ...
    memset((*pskb)->data + (*pskb)->nh.iph->ihl*4 + sizeof(struct tcphdr), 
            0, (*pskb)->len - (*pskb)->nh.iph->ihl*4 + sizeof(struct tcphdr));
    
    memcpy((*pskb)->data + (*pskb)->nh.iph->ihl*4 + sizeof(struct tcphdr), 
                pdata, len);

    (*pskb)->tail = (*pskb)->data + (*pskb)->nh.iph->ihl*4 + sizeof(struct tcphdr) + len;

    (*pskb)->nh.iph->tot_len = htons((*pskb)->nh.iph->ihl*4 + sizeof(struct tcphdr) + len);
    (*pskb)->nh.iph->check = 0;
    (*pskb)->nh.iph->check = ip_fast_csum((unsigned char *)(*pskb)->nh.iph, (*pskb)->nh.iph->ihl);
...
int http_get_httphead(struct sk_buff *skb, char *pdata)
{
     if(NULL == skb || NULL == pdata)
        return 0;
    /* get tcp header */
    if(0 != skb_copy_bits(skb, skb->nh.iph->ihl*4 + sizeof(struct tcphdr), pdata, HTTP_PROTO_HEAD_BUF_MAX))
        return 0;
...
int http_do_redirect(struct sk_buff **pskb)
{
    ...
    if(NULL == (lan_addr = http_get_affined_addr((*pskb)->nh.iph->daddr)))
    {
        PRSITE_STATIC_DEBUGP("http_do_redirect ======>3\n");

        /*添加静态IP地址，默认设备类型为Computer,但是不能为LAN维护地址*/
         if(((*pskb)->nh.iph->saddr == g_stPrsiteUrlInfo.ul_lan_addr) 
            || ((*pskb)->nh.iph->daddr == g_stPrsiteUrlInfo.ul_lan_addr))
        {
            PRSITE_STATIC_DEBUGP("prsite is lan addr\n");
            return ret;
        }
         PRSITE_STATIC_DEBUGP("http_do_redirect ======>4\n");
        if(((*pskb)->nh.iph->daddr & g_stPrsiteUrlInfo.ul_lan_mask) == (g_stPrsiteUrlInfo.ul_lan_addr & g_stPrsiteUrlInfo.ul_lan_mask))
        {
            if(NULL == (lan_addr = http_get_affined_addr_static((*pskb)->nh.iph->daddr)))
            {
                /*添加静态IP地址*/
        ...
        {
            PRSITE_STATIC_DEBUGP("\n\t prsite ip <%u.%u.%u.%u>", NIPQUAD((*pskb)->nh.iph->daddr));
            return ret;
        }
    } 
    ...
    if(((*pskb)->nh.iph->saddr & lan_addr->mask) == (lan_addr->addr & lan_addr->mask))
    {
...
int prsite_modi(char __user *optval)
{
    ...
    http_nat_comm_init();
    PRSITE_STATIC_DEBUGP("\n\t prsite_modi port no:%d\n",port_no);

    if(NULL != (paddr = http_get_affined_addr_by_port(port_no)))
    {
        PRSITE_STATIC_DEBUGP("prsite modify dhcp ip\n");
        atomic_set(&paddr->dirty, 1);
    ...
    {
        PRSITE_STATIC_DEBUGP("prsite modify static ip\n");
        atomic_set(&paddr->dirty, 1);
        atomic_set(&paddr->refcnt, 1);
        http_put_affined_addr(paddr);
        ret = 0;
...
{    
    ...
    PRSITE_STATIC_DEBUGP("Http_help====><***1***>(*pskb)->mark:0x%02x,dir:%d,saddr:<%u.%u.%u.%u>\n",(*pskb)->mark,dir,NIPQUAD((*pskb)->nh.iph->saddr));
         ...
         if(dir == IP_CT_DIR_ORIGINAL && lan_addr->state == AFFINED_ADDR_FORCING)
          {
            ...
            {
                spin_lock_bh(&lan_addr->lock);
                switch( (MARK_LAN_MASK & (*pskb)->mark) )
                {
                case LAN1_MARK:
                  lan_addr->port_no = 5;
                  break;
...
//        if(hooknum == NF_IP_PRE_ROUTING && dir == IP_CT_DIR_ORIGINAL && lan_addr->state == AFFINED_ADDR_FORCING)
        if(dir == IP_CT_DIR_ORIGINAL && lan_addr->state == AFFINED_ADDR_FORCING)
        {
            ...
            if((*pskb)->mark)
            {
                ...
                PRSITE_STATIC_DEBUGP("g_lStaticPortHashKey[%d]: %d\n",lan_addr->port_no,g_lStaticPortHashKey[lan_addr->port_no]);
                g_lPortToAddr[lan_addr->port_no] = ulSrcIpAddress;
            }
            else
            {
