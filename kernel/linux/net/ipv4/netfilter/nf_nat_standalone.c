...
extern int sysctl_static_nat_enable;
	  ...
	  int (*okfn)(struct sk_buff *))
{
    ...
    //begin add: HG553支持IP phone 需求－tftp报文走数据PVC, nfmark最高8位不能为0x11
    /* start of HG553_protocal : Added by c65985, 2009.12.23  static nat HG553V100R001 */
    if (hooknum == NF_IP_PRE_ROUTING && !(((*pskb)->mark ^ 0x11000000) & 0xff000000)
        && !sysctl_static_nat_enable)
    /* end of HG553_protocal : Added by c65985, 2009.12.23  static nat HG553V100R001 */
    {
        ...
        if ( master )
        {
            ...
            if ( help )
            {
                if ( help->helper )
                {
                    if ( 0 == strcmp(help->helper->name, "tftp") )
                    {
                        (*pskb)->mark &= 0x00ffffff;
                    }
                }
            }
        }
    }
    //end add: HG553支持IP phone 需求－tftp报文走数据PVC
