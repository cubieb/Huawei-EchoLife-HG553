...
static void nf_conntrack_expect_insert(struct nf_conntrack_expect *exp)
{
	...
	atomic_inc(&exp->use);
#ifdef CONFIG_MIPS_BRCM
	list_add(&exp->master_list, &exp->master->expectations);
#endif
	master_help->expecting++;
...
/* add by liliang 67187 for cone nat */
#ifdef CONFIG_SUPPORT_ATP
	seq_printf(s, "src=%u.%u.%u.%u:%hu mask=%u.%u.%u.%u:%hu -----  dst=%u.%u.%u.%u:%hu mask=%u.%u.%u.%u:%hu",
	       NIPQUAD(expect->tuple.src.u3.ip), ntohs(expect->tuple.src.u.udp.port),
	       NIPQUAD(expect->mask.src.u3.ip), ntohs(expect->mask.src.u.udp.port),
	       NIPQUAD(expect->tuple.dst.u3.ip), ntohs(expect->tuple.dst.u.udp.port),
	       NIPQUAD(expect->mask.dst.u3.ip), ntohs(expect->mask.dst.u.udp.port));
#endif
