...
static unsigned int nf_ct_udp_unreplieddns_timeout __read_mostly = 1*HZ;
	...
	if (test_bit(IPS_SEEN_REPLY_BIT, &conntrack->status)) {
		...
		*/
		...
		__u16 dport = ntohs(udph->dest);
            ...
            if (dport == 53)
                nf_ct_refresh_acct(conntrack, ctinfo, skb, nf_ct_udp_unreplieddns_timeout);
            else if(dport == 5060 || sport == 5060)
                nf_ct_refresh_acct(conntrack, ctinfo, skb, nf_ct_udp_timeout_stream);
            else   
