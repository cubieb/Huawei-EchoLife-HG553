...
static int __init nf_conntrack_http_init(void)
{
	...
	for (i = 0 ; (i < MAX_PORTS) && ports[i] ; i++) 
    {
		...
		http[i].mask.src.l3num = 0xFFFF;
		http[i].mask.src.u.tcp.port =htons(0xFFFF);
		http[i].mask.dst.protonum = 0xFF;
		http[i].max_expected = HTTP_MAX_EXPECTED;
		http[i].timeout = HTTP_TIME_OUT;
		http[i].help = nf_conntrack_http_help;
		http[i].me = THIS_MODULE;
		http[i].destroy = NULL;
		http[i].to_nfattr = NULL;
