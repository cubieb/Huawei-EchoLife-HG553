 ...
 *
 * Version:	$Id: af_packet.c,v 1.2 2010/07/10 12:36:10 l43571 Exp $
 *
 * Authors:	Ross Biro
...
/* start of support ipcheck for arp by c00126165 2008-5-14*/
...
#endif
/* end of support ipcheck for arp by c00126165 2008-5-14*/
...
#ifdef CONFIG_SUPPORT_ATP
{
    ...
    if (po->num == htons (ETH_P_ARP))
    {
        //#ifdef NFMARK_REVERSED
        //spkt->spkt_protocol = mark_lan_index(skb->nfmark);
        //#else
        spkt->spkt_protocol = mark_lan_index(skb->mark);
        //#endif /* NFMARK_REVERSED */
    }
}
#endif
/* end of support ipcheck for arp by c00126165 2008-5-14*/
...
/*暂时由协议这么写入进行，已提供给PPP进程使用正确的MAC地址，SOCK转入。
以后需要驱动提供上面dev->hard_header_parse的注册函数*/
