...
int xfrm_state_add(struct xfrm_state *x)
{
    ...
    /* start of 同步问题单 AU4D00688 2007-07-14 by d00107688 解决当存在多个SA的时候因为路由缓存的原因导致即便是有新的SA协商好了，但是还是使用那条旧的缓存的路由 */
    rt_cache_flush(-1);
    /* end of 同步问题单 AU4D00688 2007-07-14 by d00107688 解决当存在多个SA的时候因为路由缓存的原因导致即便是有新的SA协商好了，但是还是使用那条旧的缓存的路由 */
