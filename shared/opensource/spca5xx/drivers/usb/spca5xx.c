...
/* Running flag */
atomic_t g_iRunnngFlag = ATOMIC_INIT(1);

static struct workqueue_struct *spca_workqueue;
static struct work_struct spca_work;
...
void outpict_do_tasklet(void* ptr);
...
static void spca50x_isoc_irq(struct urb *urb)
{
    ...
    if (spca50x->curframe >= 0 && !s_iFlashWriteFlg && atomic_read(&g_iRunnngFlag) == 1) {
...
static int spca50x_move_data(struct usb_spca50x *spca50x, struct urb *urb)
{
	...
	int st = urb->iso_frame_desc[i].status;
        ...
        struct spca50x_frame *frame = NULL;	//Pointer to frame data
	int sequenceNumber;
	int sof;
	int iPix;			//Offset of pixel data in the ISO packet
        if (st){

        if (++spca50x->visoc_errors > 20)
        {
            break;
        }
	...
	if (sequenceNumber == 0) {
		...
		if (frame->format != VIDEO_PALETTE_RAW_JPEG) {
		 /* overflow ? */
            ...
            INIT_WORK(&spca_work, outpict_do_tasklet, (void *)frame);
            queue_work(spca_workqueue, &spca_work);
...
static void spca5xx_close(struct video_device *vdev)
{
	...
	printk("spca5xx: close before down\n");
    down(&spca50x->lock);

    spca50x->user--;
    printk("spca5xx: after  down %d\n",spca50x->user);
    spca50x->curframe = -1;
    spca50x->userPid = 0;
    if (spca50x->present) {
	spca50x_stop_isoc(spca50x);
...
spca5xx_ioctl(struct video_device *vdev, unsigned int cmd, void *arg)
{
    ...
    case VIDIOCSSTART:
        atomic_set(&g_iRunnngFlag, 1);
        return 0;
    case VIDIOCSSTOP:
        atomic_set(&g_iRunnngFlag, 0);
        return 0;
    case VIDIOCSPID:
    {
        spca50x->userPid = *(int*)arg;
        break;
    }
...
static void spca5xx_disconnect(struct usb_device *dev, void *ptr)
{
    ...
    struct task_struct* pstMotion = NULL;
    ...
    if (spca50x->userPid)
    {
        pstMotion = find_task_by_pid(spca50x->userPid);
        if (pstMotion)
            send_sig(SIGKILL, pstMotion, 0);
    }
...
int spca5xx_read_procmem(char *buf, char **start, off_t offset,
                   int count, int *eof, void *data)
{
	int len = 0;
	len += sprintf(buf, "%d\r\n", g_iCamSt);
...
static void spca5xx_remove_proc(void)
{
	/* no problem if it was not registered */
	remove_proc_entry("webcam", NULL /* parent dir */);
}
...
static int __init usb_spca5xx_init(void)
{
    ...
    spca_workqueue = create_singlethread_workqueue("spca");

    create_proc_read_entry("webcam", 0 /* default mode */,
			NULL /* parent dir */, spca5xx_read_procmem,
			NULL /* client data */);    
...
static void __exit usb_spca5xx_exit(void)
{
    ...
    flush_workqueue(spca_workqueue);
	destroy_workqueue(spca_workqueue);
