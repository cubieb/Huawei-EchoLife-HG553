...
static int br_cmd_adddhcpotion(char** argv)
{
 ...
 int br_cmd_add_mccheck(char** argv)
{
...
static int br_cmd_adddhcpvlan(char** argv)
{
	   int err;
	   printf("br_cmd_adddhcpvlan \n");
	   
	switch (err = br_add_dhcpvlan(argv[1])) {
	case 0:
		return 0;
	default:
		fprintf(stderr, "brctl br_cmd_adddhcpvlan %s failed: %s \n",
			argv[1], strerror(err));
		return 1;
	}
}
...
/*end  of 增加添加内核brctl option60的值 by s53329 at  20070731*/
	...
	/* BEGIN: Modified by w00135358, 2010/01/27: bug112 of ip phone arp HG553V100R001*/
	{ 1, "adddhcpvlan", br_cmd_adddhcpvlan,
	  "add dhcpvlan for IP Phone" },
	/* END: Modified by w00135358, 2010/01/27: bug112 of ip phone arp HG553V100R001*/
