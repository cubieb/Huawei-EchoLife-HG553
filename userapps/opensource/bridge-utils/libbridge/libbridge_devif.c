...
int br_get_vlan(const char *bridge, unsigned short vid, unsigned short* pports)
{
	struct ifreq ifr;
	int err;
	unsigned short pts_mask;
	unsigned long args[4] = { BRCTL_GET_VLAN, (unsigned long)vid, &pts_mask, 0 };
...
int br_get_portno(const char *brname, const char *ifname)
{
    return get_portno(brname, ifname);
}
