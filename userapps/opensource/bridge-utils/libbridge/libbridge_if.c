...
int br_add_dhcpoption(const char *dhcpoption)
{
       ...
       strncpy(option, dhcpoption, DHCP_OPTION60);
       int err = 0;
...
int br_add_dhcpvlan(const char *dhcpvlan)
{
	int err = 0;
    printf("enter br_add_dhcpvlan \n");

	char vlanId[IFNAMSIZ];
	strncpy(vlanId, dhcpvlan, IFNAMSIZ);

	if(atoi(dhcpvlan) < 0 || atoi(dhcpvlan) > 4094)
	{
		printf("wrong vlan for ip phone!\n");
        return EINVAL;
	}
	err = ioctl(br_socket_fd, SIOCBRADDDHCPVLAN, vlanId);
	return err < 0 ? errno : 0;
}
