...
{
    int tagChipId = 0;
    unsigned int chipId = (int) boardIoctl(BOARD_IOCTL_GET_CHIP_ID, 0, "", 0, 0, "") ;
    int result;
...
int myRead(char *outBuf, int inLen)
{
   ...
   if (!gXmlBuf) 
   {
      ...
      psiSize = boardIoctl(BOARD_IOCTL_GET_PSI_SIZE, 0, "", 0, 0, "");
      if ((gXmlBuf = xmalloc(psiSize)) == NULL) 
      {
         printf("Failed to allocate %d bytes of memory.\n", psiSize);
         return -1;
      }
	boardIoctl(BOARD_IOCTL_FLASH_READ, PERSISTENT, gXmlBuf, psiSize, 0, "");
...
int myWrite(char *inBuf, int inBufLen)
{
   if (glbCurPtr == NULL) 
   {
      ...
      {   
         glbTotalAllocatedSize = boardIoctl(BOARD_IOCTL_FLASH_READ, FLASH_SIZE, "", 0, 0, "") + TOKEN_LEN;
