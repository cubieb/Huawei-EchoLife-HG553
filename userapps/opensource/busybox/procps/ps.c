...
extern int ps_main(int argc, char **argv)
{
	...
	while ((p = procps_scan(1)) != 0) {
		...
		// brcm begin
		/* start of maintain tmpfile by liuzhijie 00028714 2006年5月22日
		...
		*/
		if ( strncmp(p->short_cmd,"telnetd",7)==0 || 
			 ...
			 strncmp(p->short_cmd,"tr69c",5)==0 || 
			 strncmp(p->short_cmd,"snmp",4)==0 ||
			  strncmp(p->short_cmd,"dnspr",5)==0 ||
             /* start of maintain Auto Upgrade by zhangliang 60003055 2006年5月22日 */
             strncmp(p->short_cmd,"augd",4)==0 || 
             /* end of maintain Auto Upgrade by zhangliang 60003055 2006年5月22日 */
             strncmp(p->short_cmd,"msgproc",7)==0 ||
             strncmp(p->short_cmd,"alive",5)==0 ||
			 strncmp(p->short_cmd,"tmpfile",7)==0 ||
			 strncmp(p->short_cmd,"bftpd",5)==0) {
		/* end of maintain tmpfile by liuzhijie 00028714 2006年5月22日 */
				printf("%s\n",p->short_cmd);
		} else {
