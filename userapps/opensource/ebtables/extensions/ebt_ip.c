...
#define IP_SPORT     '5'
#define IP_DPORT     '6'
#define IP_SRC_RANGE '7'
#define IP_DST_RANGE '8'
...
static struct option opts[] =
{
	...
	{ "ip-src-range"        , required_argument, 0, IP_SRC_RANGE  },
	{ "ip-dst-range"        , required_argument, 0, IP_DST_RANGE  },
	{ 0 }
};
...
/*parse the ip range*/
static void parse_iprange(char *arg, struct ebt_iprange *range)
{
	char *dash;
    uint32_t addr = 0;
	...
	if (undot_ip(arg, (unsigned char *)&addr))
 		print_error("iprange match: Bad IP address `%s'\n", arg);
	range->min_ip = addr;

	if (dash) 
    {
        if (undot_ip((dash+1), (unsigned char *)&addr))
			print_error("iprange match: Bad IP address `%s'\n", dash+1);
		range->max_ip = addr;
	} 
    else
...
"[!] --ip-src-range ip-ip        Match source IP in the specified range\n"
"[!] --ip-dst-range ip-ip        Match destination IP in the specified range\n");
}
...
#define OPT_IPRANGE_SRC  0x40
#define OPT_IPRANGE_DST  0x80
   ...
   unsigned int *flags, struct ebt_entry_match **match)
{
    ...
    case IP_SRC_RANGE:
		check_option(flags, OPT_IPRANGE_SRC);
		ipinfo->bitmask |= EBT_IPRANGE_SRC;

	case IP_DST_RANGE:
		if (c == IP_DST_RANGE) 
        {
			check_option(flags, OPT_IPRANGE_DST);
			ipinfo->bitmask |= EBT_IPRANGE_DST;
		}
		if (check_inverse(optarg)) 
        {
			if (c == IP_SRC_RANGE)
				ipinfo->invflags |= EBT_IPRANGE_SRC;
			else
				ipinfo->invflags |= EBT_IPRANGE_DST;
		}

		if (optind > argc)
			print_error("Missing IP address range argument");
        
		if (c == IP_SRC_RANGE)
			parse_iprange(argv[optind - 1], &ipinfo->src);
		else
			parse_iprange(argv[optind - 1], &ipinfo->dst);
		break;
   ...
   const struct ebt_entry_match *match)
{
    ...
    if (ipinfo->bitmask & EBT_IPRANGE_SRC){
		printf("--ip-src-range ");
		if (ipinfo->invflags & EBT_IPRANGE_SRC) {
			printf("! ");
		}
		for (j = 0; j < 4; j++)
			printf("%d%s", ((unsigned char *)&ipinfo->src.min_ip)[j],
			   (j == 3) ? "" : ".");
		printf("-");
        for (j = 0; j < 4; j++)
			printf("%d%s", ((unsigned char *)&ipinfo->src.max_ip)[j],
			   (j == 3) ? " " : ".");
	}
    if (ipinfo->bitmask & EBT_IPRANGE_DST){
		printf("--ip-dst-range ");
		if (ipinfo->invflags & EBT_IPRANGE_DST) {
			printf("! ");
		}
		for (j = 0; j < 4; j++)
			printf("%d%s", ((unsigned char *)&ipinfo->dst.min_ip)[j],
			   (j == 3) ? "" : ".");
		printf("-");
        for (j = 0; j < 4; j++)
			printf("%d%s", ((unsigned char *)&ipinfo->dst.max_ip)[j],
			   (j == 3) ? " " : ".");
	}
}
