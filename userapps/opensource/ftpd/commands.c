...
/*start of VDF 2008.7.12 V100R001C02B021 AU8D00791 l68693*/
/* 该前缀用于判断是否为USB存储目录,一定要与usbmount中的USB存储挂载目标目录保持一致 */
#define USB_STORAGE_PREFIX   "USBDisk_"
/*endi of VDF 2008.7.12 V100R001C02B021 AU8D00791 l68693*/
...
void control_printf(char success, char *format, ...)
{
    ...
    char output_buffer[MAX_STRING_LENGTH+32];
    va_list val;
    va_start(val, format);
...
void do_stor(char *filename, int flags)
{
    ...
    /*start of VDF 2008.7.12 V100R001C02B021 AU8D00791 l68693*/
    /*上传文件路径如果指定为内存目录，则禁止非USB存储目录上传*/
	if (memcmp(userinfo.pw_dir,"/var/mnt/"USB_STORAGE_PREFIX,17))
	{
		 if (mapped && strncmp(mapped, "/"USB_STORAGE_PREFIX, 9))
		 {
		      control_printf(SL_FAILURE, "553 Error: mapped %s. Only support to create file in USB storage (directory beginning with USBDisk_).", mapped);
		      free(mapped);
		      mapped = NULL;
		      return;
		 }
	}
    /*end of VDF 2008.7.12 V100R001C02B021 AU8D00791 l68693*/
...
//           printf("$$$$$$ Write bytes %d and errno is %d\n", nWRBytes, errno);
//	       bftpd_log("Unable to create buffer to receive file.\n", 0);
	       if (nWRBytes == -1)
	       	{	       	    
...
void command_mkd(char *dirname)
{
    ...
    /*start of VDF 2008.7.12 V100R001C02B021 AU8D00791 l68693*/
    /* 如果不是HG下挂USB设备,则要判断并禁止根目录写入 */
    if (!strstr(userinfo.pw_dir, "/var/mnt/"USB_STORAGE_PREFIX))
    {
        pcwd = bftpd_cwd_getcwd();
        /*不允许在非USB存储设备的根目录下创建目录*/
        if (strcmp(pcwd, "/"))
        {
            iError = 0;
        }
	...
	if (1 == iError) {
        ...
        control_printf(SL_FAILURE, "451 Error: Only support create directory in USB storage (directory beginning with USBDisk_).");
    } else if (mkdir(mapped, 0755)) {
		bftpd_log("Error: '%s' while trying to create directory '%s'.\n",
				  strerror(errno), dirname);
		control_printf(SL_FAILURE, "451 Error: %s.", strerror(errno));
	} else {
