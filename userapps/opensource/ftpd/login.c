...
int bftpd_login(char *password)
{
	...
	if (strcasecmp(config_getoption("DO_CHROOT"), "no")) {
        ...
        /* HUAWEI VHG WangXinfeng 69233 2008-07-05 Add: Forbid non-FTP user access FTP service begin */
        if (strcmp(userinfo.pw_gecos, "FTP user"))
        {
            control_printf(SL_FAILURE, "The current user \"%s\" is not a valid FTP user!", 
              userinfo.pw_name);
            exit(0);
        }
        /* HUAWEI VHG WangXinfeng 69233 2008-07-05 Add: Forbid non-FTP user access FTP service end */
