...
"--icase 					Match string insensetive\n",
...
static void string_init(/*struct xt_entry_match *m*/struct ipt_entry_match *m, unsigned int *nfcache)
{
	struct xt_string_info *i = (struct xt_string_info *) m->data;

    memset(i, 0, sizeof(struct xt_string_info));
...
#define FROM   0x4
#define TO     0x8
/* BEGIN: Added by weishi kf33269, 2010/11/24   PN:url大小写*/
#define ICASE  0x10
/* END:   Added by weishi kf33269, 2010/11/24 */
...
/* BEGIN: Modified by weishi kf33269, 2010/11/24   PN:url大小写*/
		if (invert)
		{
			stringinfo->invert |= XT_STRING_FLAG_INVERT;
		}
/* END:   Modified by weishi kf33269, 2010/11/24 */
...
/* END:   Modified by weishi kf33269, 2010/11/24 */
	...
	case '6':
		if (*flags & ICASE)
			exit_error(PARAMETER_PROBLEM,
				   "Can't specify multiple --icase");
		*flags |= ICASE;
		stringinfo->invert |= XT_STRING_FLAG_IGNORECASE;
		break;
	default:
		return 0;
	}
...
/* BEGIN: Modified by weishi kf33269, 2010/11/25   PN:url路径大小写不敏感*/
	if (is_hex_string(info->pattern, info->patlen)) {
		printf("STRING match %s", (info->invert & XT_STRING_FLAG_INVERT) ? "!" : "");
...
/* BEGIN: Added by weishi kf33269, 2010/11/24   PN:url大小写*/
	if (info->invert & XT_STRING_FLAG_IGNORECASE)
		printf("ICASE ");
/* END:   Added by weishi kf33269, 2010/11/24 */
...
static void string_save
                        /*(const void *ip, const struct xt_entry_match *match)*/
                        (const struct ipt_ip *ip, const struct ipt_entry_match *match)
{
	const struct xt_string_info *info =
	    (const struct xt_string_info*) match->data;

	if (is_hex_string(info->pattern, info->patlen)) {
	/* BEGIN: Modified by weishi kf33269, 2010/11/26   PN:url路径大小写不敏感*/
		printf("--hex-string %s", (info->invert & XT_STRING_FLAG_INVERT) ? "! ": "");
	/* END:   Modified by weishi kf33269, 2010/11/26 */
		print_hex_string(info->pattern, info->patlen);
	} else {
	/* BEGIN: Modified by weishi kf33269, 2010/11/26   PN:url路径大小写不敏感*/
		printf("--string %s", (info->invert & XT_STRING_FLAG_INVERT) ? "! ": "");
	/* END:   Modified by weishi kf33269, 2010/11/26 */
		print_string(info->pattern, info->patlen);
	}
...
static struct iptables_match string_match = {
    .name		= "string",
    .version		= IPTABLES_VERSION,
    .size		= IPT_ALIGN(sizeof(struct xt_string_info)),
