...
static void on_motion_detected_command(struct context *cnt,
            ...
            struct tm *tm ATTRIBUTE_UNUSED)
{
    MLOG("event.c: in on_motion_detected_command \n");
	if (cnt->conf.on_motion_detected)
    {   
        MLOG("event.c : detect motion ==============\n");
            ...
            void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	...
	if (conf->motion_img) {
        //z67625
		if(0 == put_picture(cnt, fullfilenamem, cnt->imgs.out, FTYPE_IMAGE_MOTION))
        {
            MDEBUG("put picture error,type is FTYPE_IMAGE_MOTION\n");
        }      
	}
	if (cnt->new_img==NEWIMG_ON || cnt->preview_shot) {
        //add by z67625
        if(strcasecmp(cnt->conf.filepath,"/dev/null"))
        {
            put_picture(cnt, fullfilename, newimg, FTYPE_IMAGE);
            MLOG("put picture ,type is FTYPE_IMAGE\n");
        } 
        else
        { 
            ...
            if(camara)
            {
                 #define OUTPUTPATH "/var/motion/photocap.jpg"
                 MLOG(" out put a new img");
                 put_picture(cnt, OUTPUTPATH, newimg, FTYPE_IMAGE);
                 camara = 0;
            }
        }
        //add by z67625
            ...
            void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	...
	if ( strcmp(cnt->conf.snappath, "lastsnap") ) {
        ...
        //z67625
		if(0 == put_picture(cnt, fullfilename, img, FTYPE_IMAGE_SNAPSHOT))
        {
            MDEBUG("put_picture error,type is FTYPE_IMAGE_SNAPSHOT\n");
        }      
		...
		if(0 == put_picture(cnt, fullfilename, img, FTYPE_IMAGE_SNAPSHOT))
        {
            MDEBUG("put_picture error 2 type is FTYPE_IMAGE_SNAPSHOT");
        }      
	}
