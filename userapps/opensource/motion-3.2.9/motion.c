...
int camara = 0;
//是否需要监控
int need_motion_detected = 0;
//是否启用远程访问控制
int remote_enable = 0;
//远程访问用户名密码
char *remote_pass = NULL;
//调试开关
int debug = 0;
//add by z67625存储dhcp ip地址段
char lanIp[2][16];
...
static void sig_handler(int signo)
{
	...
	switch(signo) {
        ...
        case SIGUSR2:
            if(cnt_list && sigpass)
            {
                cnt_list = get_userconf(cnt_list, 0);
            }
            if(restart && sigpass)
            {
                    if (cnt_list) {
...
static int motion_init(struct context *cnt)
{
	...
	if (cnt->video_dev > 0) {
        ...
        if(cnt->conf.netcam_url == NULL)
        {
            ...
            {
                MDEBUG("\nstart video_dev failed\n");
                return -1;
            }
            else
            {
                MLOG("start video_dev success\n");
            }
        }
		for (i = 0; i < 10; i++) {
...
static void *motion_loop(void *arg)
{
    ...
    //add by z67625
    MLOG("will start while\n");
	while (!cnt->finish ||( cnt->makemovie && need_motion_detected)) {
...
if(need_motion_detected)
{
	...
	/***** MOTION LOOP - VIDEO LOOPBACK SECTION *****/
		...
		if (cnt->conf.setup_mode) {
    		...
    		//add by z67625
    		if(camara && !need_motion_detected)
            {
                event(cnt, EVENT_IMAGE_DETECTED, newimg, NULL, NULL, cnt->currenttime_tm);
            }      
            if(camara)
...
static void cntlist_create(int argc, char *argv[])
{
	...
	cnt_list = conf_load(cnt_list);//获取配置
	//add by z67625
	cnt_list = get_userconf(cnt_list, 1); // 获取用户配置
	get_lanconf();//add by z67625 获取dhcpip
}
...
static void motion_startup(int daemonize, int argc, char *argv[])
{
	 ...
	 * configuration.
	 */
	cntlist_create(argc, argv);//获取配置

	motion_log(LOG_INFO, 0, "Motion "VERSION" Started");

	initialize_chars();//初始化字符集
...
static void setup_signals(struct sigaction *sig_handler_action, struct sigaction *sigchild_action)
{
    ...
    //add by z67625
    sigaction(SIGUSR2, sig_handler_action, NULL);//修改了配置文件
    sigaction(SIGILL, sig_handler_action, NULL);//拍照
    sigaction(SIGURG,sig_handler_action,NULL);//调试开关
}
...
int main (int argc, char **argv)
{
	...
	motion_startup(1, argc, argv);//获取配置，初始化
