...
int put_picture(struct context *cnt, char *file, unsigned char *image, int ftype)
{
	FILE *picture;
...
void preview_best(struct context *cnt)
{
	...
	if(cnt->preview_max){
        ...
        //z67625
		if(0 == put_picture(cnt, previewname, cnt->imgs.preview_buffer , FTYPE_IMAGE))
        {
            //printf("%s %d FTYPE_IMAGE",__FUNCTION__,__LINE__);
        }      
	}
}
