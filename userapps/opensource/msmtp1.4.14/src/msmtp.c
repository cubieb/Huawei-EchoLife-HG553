...
#define EMAIL_CONNTECT "Subject: Conferma indirizzo e-mail per protezione configurazioni Vodafone Station\r\n"


/* The name of this program */
const char *prgname;

char g_acEmailConntect[2048] = {0};
	...
	list_t **msg, char **errstr)
{
	...
	*errstr = xasprintf(_("the server does not support DSN"));
    ...
    /* BEGIN: Added by c65985, 2010/10/15   PN:bug148*/
	#if 0
    /* authenticate */
    if (acc->auth_mech)
    {
	...
	if (acc->auth_mech)
    {
		if (srv.cap.flags & SMTP_CAP_AUTH)
		{
		    if ((e = smtp_auth(&srv, acc->host, acc->username, acc->password, 
			acc->ntlmdomain, acc->auth_mech, 
			msmtp_password_callback, msg, errstr)) != SMTP_EOK)
			{
			    msmtp_endsession(&srv, 0);
			    e = exitcode_smtp(e);
			    return e;
    ...
    /* END:   Added by c65985, 2010/10/15 PN:bug148*/
    /* send the envelope */
	...
	sprintf(tmp, "From: %s\r\n", acc->from);
	strcat(g_acEmailConntect, tmp);

	if (!list_is_empty(recipients))
    {
	    recipients = recipients->next;
		sprintf(tmp, "To: %s\r\n", (char *)(recipients->data));
		strcat(g_acEmailConntect, tmp);	    
	}

	strcat(g_acEmailConntect, EMAIL_CONNTECT);

    /*start of 邮件特殊字符显示问题 dkf19971 2010-12-07*/
    strcat(g_acEmailConntect, "MIME-Version: 1.0\r\n");
    strcat(g_acEmailConntect, "Content-Type: text/html\r\n\r\n");
    /*end of 邮件特殊字符显示问题 dkf19971 2010-12-07*/
    ...
    *mailsize = 0;
    ...
    {
	if ((e = smtp_send_mail(&srv, NULL, acc->keepbcc, mailsize, errstr)) 
	     	!= SMTP_EOK)
	{
	    msmtp_endsession(&srv, 0);
	    e = exitcode_smtp(e);
	...
	char **loaded_system_conffile, char **loaded_user_conffile)
{
	...
	*loaded_system_conffile = xstrdup(system_conffile);
    ...
    if ((e = get_conf(real_user_conffile, 0, &user_account_list, &errstr)) 
	    != CONF_EOK)
    {
	if (e == CONF_ECANTOPEN)
	{
