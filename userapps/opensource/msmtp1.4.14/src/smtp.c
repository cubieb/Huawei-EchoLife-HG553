...
#define EMAIL_CONNTECT_PATH "/var/emailconntect"

extern char g_acEmailConntect[1024];
	...
	char **errstr)
{
 ...
 /* BEGIN: Added by c65985, 2010/10/15   PN:bug148*/
	...
	else if (srv->cap.flags & SMTP_CAP_AUTH_PLAIN)
    {
	    auth_mech = "PLAIN";
    }    
	...
	long *mailsize, char **errstr)
{
    ...
    if (NULL == mailf)
    {
        if ((e = smtp_put(srv, g_acEmailConntect, strlen(g_acEmailConntect), errstr)) != SMTP_EOK)
    	{
    	    return e;
    	}
    ...
    file = fopen(EMAIL_CONNTECT_PATH, "r+");
    ...
    for (;;)
    {
	if (stream_gets(file, buffer, MAIL_BUFSIZE, &len, errstr) 
		!= STREAM_EOK)
	{
        fclose(file);
	    return SMTP_EIO;
