...
int at_sm_modem_submit_at(int modem_fd, char* at_dial_str)
{
    ...
    {
       /*star 2009-04-06 w45260:数据卡采用高通新基线后,将ATX的默认值从0修改为1，导致命令上报带有扩展字段
       （速率信息,例如:CONNECT 7200000 ).这种情况下会出现不能进行数据拨号的现象.
        原因:网关对数据卡的响应信息匹配太严格,导致网关主动中断拨号.
        解决方法:网关放宽匹配条件兼容带拓展字段的情况*/
        //report_msg = "\r\nCONNECT\r\n";
        report_msg = "CONNECT";
		/*end 2009-04-06*/
