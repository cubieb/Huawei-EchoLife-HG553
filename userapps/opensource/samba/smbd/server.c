...
{
    BOOL ret;
//printf("\n==reload_services start==file:%s,line:%d===\n",__FILE__,__LINE__);
    if (lp_loaded()) {
  //      printf("\n===file:%s,line:%d===\n",__FILE__,__LINE__);
        pstring fname;
   ...
   // printf("\n===file:%s,line:%d===\n",__FILE__,__LINE__);
    ...
    reopen_logs();
    //printf("\n=下面开始调用load_interfaces==file:%s,line:%d===\n",__FILE__,__LINE__);
/*暂时未能调试通过interfaces，故先屏蔽之，调试后面部分20081007*/
   load_interfaces();
   // printf("\n=load_interfaces end==file:%s,line:%d===\n",__FILE__,__LINE__);
    {
        if (smbd_server_fd() != -1) {     
     //        printf("\n===file:%s,line:%d===\n",__FILE__,__LINE__);
            set_socket_options(smbd_server_fd(),"SO_KEEPALIVE");
    ...
    set_current_service(NULL,True);
     //printf("\n===file:%s,line:%d===\n",__FILE__,__LINE__);
    //printf("\n==reload_services end==file:%s,line:%d===\n",__FILE__,__LINE__);
 ...
 int main(int argc,const char *argv[])
{
    ...
    //printf("\n==set_remote_machine_name end==file:%s,line:%d===\n",__FILE__,__LINE__);
...
//printf("\n====file:%s,line:%d===\n",__FILE__,__LINE__);
  ...
  //      printf("\n==exit from here==file:%s,line:%d===\n",__FILE__,__LINE__);
    ...
    */
//printf("\n==下面进行if判断==file:%s,line:%d===\n",__FILE__,__LINE__);
    /*由于interfaces未能改正，故先改变这里的条件，调试后面部分于20081007**/

    if (!reload_services(False))
    {
  //      printf("\n===exir from here=file:%s,line:%d===\n",__FILE__,__LINE__);
        return(-1);
    }
...
//printf("\n====file:%s,line:%d===\n",__FILE__,__LINE__);
    ...
    if (!is_daemon && !is_a_socket(0)) {
      //  printf("\n====file:%s,line:%d===\n",__FILE__,__LINE__);
         ...
         */
        //printf("\n===is_daemon=true===file:%s,line:%d===\n",__FILE__,__LINE__);
        is_daemon = True;
    }

    if (is_daemon && !interactive) {
        //printf("\n==is_daemon && !interactive==file:%s,line:%d===\n",__FILE__,__LINE__);
    ...
    */ 
    ...
    if(!get_global_sam_sid()) {
        ...
        //printf("\n===exit from here ERROR: Samba cannot create a SAM SID=file:%s,line:%d===\n",__FILE__,__LINE__);
        exit(1);
    }
    //printf("\n====file:%s,line:%d===\n",__FILE__,__LINE__);
    ...
    smbd_process();
    //printf("\n==smbd_process 调用end==file:%s,line:%d===\n",__FILE__,__LINE__);

    namecache_shutdown(); 
    //printf("\n====file:%s,line:%d===\n",__FILE__,__LINE__);
    exit_server("normal exit");
    //printf("\n==the end!!!==file:%s,line:%d===\n",__FILE__,__LINE__);
    return(0);
}
