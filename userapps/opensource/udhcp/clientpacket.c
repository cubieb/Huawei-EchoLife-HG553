...
static void init_packet(struct dhcpMessage *packet, char type)
{
	...
	boardIoctl(BOARD_IOCTL_EQUIPMENT_TEST, GET_SERIAL_NUMBER, serial, MAX_SERIAL_NUM_LEN+1, 0, "");
    	boardIoctl(BOARD_IOCTL_EQUIPMENT_TEST, GET_BASE_MAC_ADDRESS, macaddr, 6, 0, "");
    	//sprintf(clientid, "%02x%02x%02x-%s", (unsigned char)macaddr[0], (unsigned char)macaddr[1], (unsigned char)macaddr[2],serial);
    	sprintf(clientid, "%s", serial);
        #endif
        while(!strlen(serial))
        {
            boardIoctl(BOARD_IOCTL_EQUIPMENT_TEST, GET_SERIAL_NUMBER, serial, MAX_SERIAL_NUM_LEN+1, 0, "");
...
int send_renew(unsigned long xid, unsigned long server, unsigned long ciaddr)
{
	...
	if (server) 
	{
		ret = kernel_packet(&packet, ciaddr, CLIENT_PORT, server, SERVER_PORT, client_config.Interface);
	}
	else
	{
...
int send_release(unsigned long server, unsigned long ciaddr)
{
	...
	return kernel_packet(&packet, ciaddr, CLIENT_PORT, server, SERVER_PORT, client_config.Interface);
}
