...
extern int TR069_SendMessage(long msg_type, void *msg_content, int content_len);
// brcm
...
extern char gGwIp[ICMP_DHCP_AUTOACL_MAX][32];
...
/*end of HG553 2008.03.29 V100R001C02B010 AU8D00340 by yepeng */
    /*start of 增加保存各接口默认网关功能by s53329  at  20071220*/
     ...
     if (strlen(session_path) > 0)
     {
         ...
         //begin:如果dhcp服务器更新了dns配置，使之在网关上立即生效20090714
        ...
        {
            msg.mtype = DNSSRV_DHCP_TYPE;
            memset(msg.mtext, 0, sizeof(msg.mtext));
            strncpy(msg.mtext, session_path, sizeof(msg.mtext) - 1);
            if (-1 == msgsnd(msgid, &msg, sizeof(msg) - sizeof(long), 0))
            {
                perror("dnssvr");
            }
        }
         //end:如果dhcp服务器更新了dns配置，使之在网关上立即生效20090714
    }
	if (strlen(session_path) > 0)
	{
   ...
   if(fsRoute != NULL) 
   {
      ...
      while(fgets(line, sizeof(line), fsRoute)) 
      {
		 ...
		 if (strcmp(gtwy, "0.0.0.0") != 0) 
		 {
			 ...
			 {
				if(0 == strcmp(gtwy, gGwIp[i]))
				{
				  printf("[%s,%d] same gateway from option121:%s======\n",__func__,__LINE__,gtwy);				
				  break;
				}
			 }
			 if(i == entryIndex && entryIndex >= 0 && entryIndex < ICMP_DHCP_AUTOACL_MAX)
			 {
			 	strncpy(gGwIp[entryIndex], gtwy, 16);
			 	entryIndex++;
			 }
			 /* END: Added by KF19989, 2009/11/19: Automatic ACL */
         }
